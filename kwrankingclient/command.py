...
class KwrankingCommand(OpenStackCommand):

    """Base Kwranking CLI command."""
    api = 'ranking'
    log = logging.getLogger(__name__ + '.KwrankingCommand')
    values_specs = []
    json_indent = None
    resource = None
    allow_names = True
    ...
    def __init__(self, app, app_args):
        super(KwrankingCommand, self).__init__(app, app_args)
    ...
    def get_parser(self, prog_name):
        parser = super(KwrankingCommand, self).get_parser(prog_name)
        return parser
...
class CreateCommand(KwrankingCommand, show.ShowOne):
    ...
    def get_data(self, parsed_args):
        ...
        kwranking_client = self.get_client()
        body = self.args2body(parsed_args)
        resource_manager = getattr(kwranking_client, self.resource)
...
class UpdateCommand(KwrankingCommand):
    ...
    def run(self, parsed_args):
        ...
        body = self.args2body(parsed_args)
        if self.allow_names:
            res_id = utils.find_resource_id_by_name_or_id(kwranking_client,
                                                          self.resource,
                                                          parsed_args.id)
        else:
...
class DeleteCommand(KwrankingCommand):
...
class ListCommand(KwrankingCommand, lister.Lister):
    ...
    def args2body(self, parsed_args):
        ...
        if parsed_args.sort_by:
            if parsed_args.sort_by in self.list_columns:
                ...
                raise exception.KwrankingClientException(msg)
        return params
    ...
    def retrieve_list(self, parsed_args):
        """Retrieve a list of resources from Kwranking server"""
        kwranking_client = self.get_client()
        body = self.args2body(parsed_args)
        resource_manager = getattr(kwranking_client, self.resource)
...
class ShowCommand(KwrankingCommand, show.ShowOne):
