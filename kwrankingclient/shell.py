...
"""
Command-line interface to the Kwranking APIs
"""
...
from kwrankingclient import client as kwranking_client
from kwrankingclient import exception
from kwrankingclient.openstack.common import strutils
from kwrankingclient import utils
from kwrankingclient.v1.shell_commands import nodes
from kwrankingclient import version as base_version

COMMANDS_V1 = {
    'host-rank': nodes.RankNodes,
    'host-add': nodes.AddNode,
    'host-show': nodes.ShowNode,
    'host-list': nodes.ListNode,
}
...
class KwrankingShell(app.App):
    """Manager class for the Kwranking CLI."""
    CONSOLE_MESSAGE_FORMAT = '%(message)s'
    DEBUG_MESSAGE_FORMAT = '%(levelname)s: %(name)s %(message)s'
    log = logging.getLogger(__name__)

    def __init__(self):
        super(KwrankingShell, self).__init__(
            description=__doc__.strip(),
            version=VERSION,
            command_manager=commandmanager.CommandManager('kwranking.cli'), )
    ...
    def build_option_parser(self, description, version, argparse_kwargs=None):
        ...
        parser.add_argument(
            '--insecure',
            action='store_true',
            default=env('KWRANKINGCLIENT_INSECURE', default=False),
            help="Explicitly allow kwrankingclient to perform \"insecure\" "
    ...
    def authenticate_user(self):
        ...
        if auth:
            try:
                kwranking_url = keystone.service_catalog.url_for(
                    service_type='ranking'
                )
            except keystone_exceptions.EndpointNotFound:
                raise exception.NoKwrankingEndpoint()
        else:
            raise exception.NotAuthorized("User %s is not authorized." %
                                          self.options.os_username)

        client = kwranking_client.Client(1,
                                       kwranking_url=kwranking_url,
    ...
    def initialize_app(self, argv):
        ...
        * validate authentication info
        ...
        super(KwrankingShell, self).initialize_app(argv)
...
def main(argv=sys.argv[1:]):
    try:
        return KwrankingShell().run(map(strutils.safe_decode, argv))
    except exception.KwrankingClientException:
        return 1
