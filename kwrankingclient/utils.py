    ...
    if is_id:
        ...
        for resource in resources:
            if resource['id'] == name_or_id:
                return name_or_id
        raise exception.KwrankingClientException('No resource found with ID %s' %
                                               name_or_id)
...
def _find_resource_id_by_name(client, resource, name):
    ...
    if len(named_resources) > 1:
        ...
        raise exception.KwrankingClientException(message=message,
                                               status_code=404)
...
def from_elapsed_time_to_seconds(elapsed_time):
    ...
    if is_elapsed_time is None:
        raise exception.KwrankingClientException(_("Invalid time "
                                                 "format for option."))
